            ...
            if item.filename:
                ...
                if not rag_locks[base_name].locked():
                    async with rag_locks[base_name]:
                        ...
                        user_message = {"role": "user", "content": item.request}
                        ...
                        try:
                            ...
                            await loop.run_in_executor(
                                executor,
                                chromadb_store.save_rag_history,
                                base_name,
                                messages
                            )
...
@app.get("/rag-history/{filename}")
async def get_rag_history(filename: str):
    """获取指定文件的RAG对话历史"""
    try:
        base_name = os.path.splitext(filename)[0]
        # 从数据库获取RAG历史记录
        messages = chromadb_store.get_rag_history(base_name)
        print(messages)
        if not messages:
            # 如果没有历史记录，返回空列表和系统消息
            return JSONResponse({
        ...
        logger.error(f"获取RAG历史记录失败: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={"error": f"获取RAG历史记录失败: {str(e)}"}
        )
