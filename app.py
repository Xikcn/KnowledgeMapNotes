...
# 创建两个独立的kg_manager
rag_manager = KgManager(agent=rag_agent, splitter=rag_splitter, embedding_model=embeddings, store=chromadb_store)
    ...
    try:
        ...
        if item.filename:
            ...
            async with rag_locks[base_name]:
                # 加载RAG相关数据
                await loop.run_in_executor(executor, rag_manager.load_store, base_name)
                logger.info(f"已加载知识图谱: {item.filename}")

                # 执行RAG流程
                rag_entity = await loop.run_in_executor(executor, rag_manager.text2entity, item.request)
                community_info = await loop.run_in_executor(executor, rag_manager.community_louvain_G, rag_entity)
                results = await loop.run_in_executor(executor, rag_manager.select_vectors, item.request, item.top_k)
                result = await loop.run_in_executor(executor, rag_agent.hybrid_rag, item.request, community_info, results, item.messages)

            return JSONResponse({"result": {"answer": result['answer'], "material": result['material']}})
